...
*--------------------------------------------------------------------------------------------
	...
	l_read=0	&& текущая позиция чтения файла
	DO WHILE m.l_read<=m.l_body
		l_read=m.l_read+1
    	fld_kus = SUBSTR(m.f_body,m.l_read,1)
    	IF !EMPTY(m.fld_kus) AND m.fld_kus#'['
    		fld_kus = GETWORDNUM(SUBSTR(m.f_body,m.l_read),1,"[")
    		l_read=m.l_read+LEN(m.fld_kus)-1
		ENDIF
    	IF m.fld_kus='['
    		IF m.ret_block=1
...
*--------------------------------------------------------------------------------------------
	...
	SCAN FOR obtype=9
		...
		FOR i=1 to 10
			...
			CASE INLIST(m.tmp_text,"SUM","MAX","MIN","CNT","AVG")			&& функции подсчета
				tmp_func=m.tmp_text
				tmp_name=ALLTRIM(SUBSTR(m.tmp_name,AT(":",m.tmp_name)+1))	&& отсекаем ф-ю
			CASE INLIST(m.tmp_text,"DAT","TDT","PN1","PN2","PN3","PN4","KOL","RUB","INT","DEC","RD0","RD1","RD2","RD3","IMG")		&& функции преобразования
				tmp_fusr=m.tmp_text
				tmp_name=ALLTRIM(SUBSTR(m.tmp_name,AT(":",m.tmp_name)+1))	&& отсекаем ф-ю
			CASE INLIST(m.tmp_text,"CON","TBL","GR1","GR2","GR3","GR4","GR5","GR6")		&& ссылка на алиас
				tmp_alia=GET_REPALIAS("["+m.tmp_text+"]")
				tmp_name=ALLTRIM(SUBSTR(m.tmp_name,AT(":",m.tmp_name)+1))	&& отсекаем ф-ю
			CASE INLIST(m.tmp_text,"HEX","RAZ","RM1","RM2","RM3","LRU","BLZ","BLK")		&& функции окончательного форматирования
				tmp_fend=m.tmp_text
				tmp_name=ALLTRIM(SUBSTR(m.tmp_name,AT(":",m.tmp_name)+1))	&& отсекаем ф-ю
...
*--------------------------------------------------------------------------------------------
	...
	SCAN FOR INLIST(obtype,1,2,3,4,5,6,7,8,10,11,12,13,14,15)		&& УПРАВЛЯЮЩИЕ ТЕГИ
		IF ASC(LEFT(beetw_text,1))=13 AND ASC(SUBSTR(beetw_text,2,1))=10
...
FUNCTION REBLD_BLOK	&& Функция обработки поля
	LOCAL i,fld_txt,fld_teg,fld_txt1,fld_kus,kolskb,readpz
	LOCAL tbl__alia,tbl__key,tbl__usl,tbl__utp,tbl__ord,tbl__add,tbl__des
	fld_txt=''
	fld_kus=''
	kolskb=1	&& количество открывающих скобок [
	readpz=0
	DO WHILE m.l_read<=m.l_body	&& считывает все, что находится до следующей закрывающей скобки с учетом того, что могут быть вложенные скобки []
		l_read=m.l_read+1
    	fld_kus=SUBSTR(m.f_body,m.l_read,1)
		readpz=m.readpz+1
		m.kolskb=m.kolskb+ICASE(m.fld_kus='[',1,m.fld_kus=']',-1,0)
		IF m.kolskb>0 OR m.fld_kus!=']'
...
*--------------------------------------------------------------------------------------------
	...
	IF m.rzd_type=8 AND LEFT(m.fld_teg,9)#"REP:START"	&& если обнаружен знак [ в постоянной части отчета
		l_read=m.l_read-m.readpz						&& возврат на позицию за символом [
...
LOCAL i,kus_text,out_hex
	...
	FOR i=1 TO LEN(m.rus_text)
		...
		IF ASC(m.kus_text)<127
			out_hex=m.out_hex+m.kus_text
		ELSE
			out_hex=m.out_hex+ LOWER("\'"+RIGHT(TRANSFORM(ASC(m.kus_text),'@0'),2))
		ENDIF
	ENDFOR
RETURN m.out_hex
*############################################################################################
